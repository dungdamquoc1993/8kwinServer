...
module.exports = function(client, data){
	if (!!data.captcha && !!data.code) {
		...
		// }else
		if (!validator.isLength(data.code, {min: 4, max: 12})) {
			client.red({notice: {title: 'LỖI', text: 'GiftCode không tồn tại !!'}});
		} else {
			var checkCaptcha = true;//new RegExp('^' + data.captcha + '$', 'i').test(client.captcha);
			if (checkCaptcha) {
				var code = ''+data.code+'';
									...
									//GiftCode.findOne({'uid': client.UID, 'type': check.type}, 'code', function(err, check2) {
										//if (!!check2) {
									 	...
									 	//}else{
											...
											check.save();
											UserInfo.findOneAndUpdate({id: client.UID}, {$inc:{red:check.red, xu:check.xu}}).exec(function(err, user){
												client.red({notice:{title:'THÀNH CÔNG',text:'Bạn nhận được: ' + (check.red > 0 ? Helpers.numberWithCommas(check.red) + ' XU' + '\n' + ' Từ gift code chúc bạn may mắn': '') /*+ (check.xu > 0 ? (check.red > 0 ? ' và ' : '') + Helpers.numberWithCommas(check.xu) + ' XU' : '')*/}, user:{red:user.red*1+check.red, xu:user.xu*1+check.xu}});
											});
										//}
									//})
								}
