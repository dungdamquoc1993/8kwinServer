...
module.exports = async function (req, res) {
    ...
    if (!!daily && daily != '') {
        ...
        let adminThuDaily = await ChuyenRed.aggregate([{ $match: { time: { $gte: new Date(fromDate), $lte: new Date(toDate) }, to: "admin", from: daily , red:{$lte:0} } }, { $group: { _id: null, total: { $sum: "$red" } } }]);
        res.json({
            status: 200,
            success: true,
            data: [
        ...
        for(var i = 0 ; i < allDailyNameWithoutAdmin.length ; i++){
            ...
            let adminThuDaily = await ChuyenRed.aggregate([{ $match: { time: { $gte: new Date(fromDate), $lte: new Date(toDate) }, to: "admin", from: item , red:{$lte:0} } }, { $group: { _id: null, total: { $sum: "$red" } } }]);
            finalResult.push({
                nickname: item,
                dailyBanUser: dailyBanUser[0] ? dailyBanUser[0].total : 0,
                dailyBanDaily: dailyBanDaily[0] ? dailyBanDaily[0].total : 0,
