		...
		if (!(bet == 100 || bet == 1000 || bet == 10000)) {
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name daily':'xu name daily', function(err, user){
				if (!user || (red && user.red < bet) || (!red && user.xu < bet)) {
					...
					if (user.daily != '')
						Helpers.pushDailyVIP({ daily: user.daily, reason: "Cộng tiền người chơi " + user.name + " chơi Kim Cương", type: true, total: bet });
					let phe = red ? 2 :4;    // Phế
					...
					HU.findOne({game:'arb', type:bet, red:red}, 'name bet min toX balans x', function(err, dataHu){
						...
						.then(result => {
									...
									if(line_win.win == 4) {
										...
										if (heso == 100) {
											...
											if (!nohu) {
												...
												bet_win += okHu;
												if (red){
													client.redT.sendInHome({pushnohu:{title:'Kim Cương', name:client.profile.name, bet:okHu}});
							...
							if (!nohu && bet_win >= bet*10) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Kim Cương', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
