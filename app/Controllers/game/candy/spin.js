...
module.exports = function(client, data){
	...
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			client.Candy = void 0 === client.Candy ? {id:'', red:red, bet:bet, bonus:null, bonusL:0, bonusWin:0, free:0} :client.Candy;
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name daily':'xu name daily', function(err, user){
				if (client.Candy.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					...
					if (user.daily != '')
						Helpers.pushDailyVIP({ daily: user.daily, reason: "Cộng tiền người chơi " + user.name + " chơi Sex & Zen", type: true, total: tongCuoc });
					...
					HU.findOne({game:'candy', type:bet, red:red}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											red && client.redT.sendInHome({pushnohu:{title:'Sex & Zen', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Sex & Zen', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
							...
							if (red) {
								...
								Candy_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({candy:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Candy.free, isFree:isFree, isBonus:client.Candy.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:user.red-tongCuoc}});
								...
								Candy_xu.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({candy:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Candy.free, isFree:isFree, isBonus:!!client.Candy.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong, phien:small.id}, user:{xu:user.xu-tongCuoc}});
									client.Candy.id = small._id.toString();
								});
							}
