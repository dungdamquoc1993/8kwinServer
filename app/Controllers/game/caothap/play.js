...
function newGame(client, data) {
	if (!!data && !!data.cuoc) {
		...
		var red  = true;	  // Loại tiền đang chơi
		if (!(cuoc == 1000 || cuoc == 10000 || cuoc == 50000 || cuoc == 100000 || cuoc == 500000)) {
			...
			UserInfo.findOne({id:client.UID}, 'red redPlay xu xuPlay name daily', function(err, user){
				if (!user || (red && user.red < cuoc) || (!red && user.xu < cuoc)) {
					...
					if (user.daily != '')
						Helpers.pushDailyVIP({ daily: user.daily, reason: "Cộng tiền người chơi " + user.name + " chơi CAO THẤP", type: true, total: cuoc });
					var addQuy = (cuoc*0.01)>>0;                 // Thêm vào hũ
					Helpers.MissionAddCurrent(client.UID, (cuoc * 0.02>>0));
