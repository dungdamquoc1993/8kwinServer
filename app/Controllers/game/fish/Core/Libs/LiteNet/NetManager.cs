...
namespace LiteNetLib
{
    ...
    public class NetManager : INetSocketListener, IConnectionRequestListener, IEnumerable<NetPeer>
    {
        ...
        public delegate object DataDeserializer(byte[] data, int pos, int length);
        /// <summary>
        /// Custom deserializer to deserialize as soon as received data (deserialize on network thread, pass to OnReceive).
        /// </summary>
            ...
            object deserializedData = null)
        {
            ...
            evt.DeserializedData = deserializedData;
            if (UnsyncedEvents)
            {
                ProcessEvent(evt);
            }
        ...
        private void ProcessEvent(NetEvent evt)
        {
            ...
            switch (evt.Type)
            {
                ...
                case NetEvent.EType.Receive:
                    _netEventListener.OnNetworkReceive(evt.Peer, evt.DataReader, evt.DeliveryMethod, evt.DeserializedData);
                    break;
        ...
        internal void ReceiveFromPeer(NetPacket packet, IPEndPoint remoteEndPoint)
        {
            ...
            try
            {
                ...
                {
                    var pos = packet.GetHeaderSize();
                    dd = Deserializer(packet.RawData, pos, packet.Size - pos);
                }
            }
            catch(Exception ex)
            {
                Logger.Error("Fail to deserialize data: " + ex.ToString());
            }
            CreateEvent(NetEvent.EType.Receive, fromPeer, deliveryMethod: deliveryMethod, readerSource: packet, deserializedData: dd);
        }
