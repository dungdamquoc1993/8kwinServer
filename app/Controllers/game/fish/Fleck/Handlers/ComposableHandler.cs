...
namespace Fleck.Handlers
{
    public class ComposableHandler : IHandler
    {
        ...
        public Func<byte[], int, int, byte[]> BinaryFrame = (x,s,l) => new byte[0];
        ...
        {
            return BinaryFrame(bytes, 0, bytes.Length);
        }
        public byte[] FrameBinary(byte[] bytes, int start, int length)
        {
            return BinaryFrame(bytes, start, length);
        }
