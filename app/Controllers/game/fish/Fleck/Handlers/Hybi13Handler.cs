    ...
    public static class Hybi13Handler
    {
        public static IHandler Create(WebSocketHttpRequest request, Action<string> onMessage, Action onClose, Action<byte[]> onBinary, Action<byte[]> onPing, Action<byte[]> onPong)
        {
            ...
            return new ComposableHandler
            {
                ...
                BinaryFrame = (s,start,length) => Hybi13Handler.FrameData(s, start, length, FrameType.Binary),
        ...
        {
            return FrameData(payload, 0, payload.Length, frameType);
        }
        public static byte[] FrameData(byte[] payload, int start, int length, FrameType frameType)
        {
            using (var memoryStream = new MemoryStream())
            {
                ...
                memoryStream.Write(payload, start, length);
