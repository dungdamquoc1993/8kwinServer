...
namespace Fleck
{
    public class WebSocketConnection : IWebSocketConnection
    {
        ...
        {
            return Send(message, 0, message.Length, Handler.FrameBinary);
        }
        public Task Send(byte[] message, int start, int length)
        {
            return Send(message, start, length, Handler.FrameBinary);
        }
        ...
        private Task Send(byte[] message, int start, int length, Func<byte[], int, int, byte[]> createFrame)
        {
            if (Handler == null)
            ...
            var bytes = createFrame(message, start, length);
