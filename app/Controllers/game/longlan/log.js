		...
		if (page < 1) {
			...
			if (red) {
				LongLan_red.countDocuments({name:client.profile.name}).exec(function(err, total){
					LongLan_red.find({name:client.profile.name}, 'id win bet line time kq', {sort:{'_id':-1}, skip: (page-1)*kmess, limit: kmess}, function(err, result) {
				...
				LongLan_xu.countDocuments({name:client.profile.name}).exec(function(err, total){
					LongLan_xu.find({name:client.profile.name}, 'id win bet line time kq', {sort:{'_id':-1}, skip: (page-1)*kmess, limit: kmess}, function(err, result) {
						Promise.all(result.map(function(obj){
							obj = obj._doc;
							delete obj._id;
							return obj;
