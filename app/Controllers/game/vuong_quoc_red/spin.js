...
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			//loadCached(bet);
			client.VuongQuocRed = void 0 === client.VuongQuocRed ? {id:'', red:red, bet:bet, bonus:null, bonusX:0, bonusL:0, bonusWin:0, free:0,lastBet:bet} :client.VuongQuocRed;
			client.VuongQuocRed.red = red;
			client.VuongQuocRed.bet = bet;
			if(client.VuongQuocRed.free > 0 && bet != client.VuongQuocRed.lastBet){
				client.red({VuongQuocRed:{notice:'Bạn không nên đổi cược khi đang free spin'}});
			}else {
				client.VuongQuocRed.lastBet = bet;
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.VuongQuocRed.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					...
					HU.findOne({game:'vuongquocred', type:bet, red:red}, {}, function(err2, dataHu){
						...
						let celSS = null;
						if(line.length<15){
							console.log("Anh Hùng, Số dòng chọn: "+line.length)
							celSS = [
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											red && client.redT.sendInHome({pushnohu:{title:'ANH HÙNG', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'ANH HÙNG', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
