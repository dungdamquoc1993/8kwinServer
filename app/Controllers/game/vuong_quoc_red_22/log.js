		...
		if (page < 1) {
			...
			if (red) {
				VuongQuocRed_red.countDocuments({name: client.profile.name}).exec(function(err, total){
					VuongQuocRed_red.find({name: client.profile.name}, 'id win bet line time', {sort:{'_id':-1}, skip: (page-1)*kmess, limit: kmess}, function(err, result) {
				...
				VuongQuocRed_xu.countDocuments({name: client.profile.name}).exec(function(err, total){
					VuongQuocRed_xu.find({name: client.profile.name}, 'id win bet line time', {sort:{'_id':-1}, skip: (page-1)*kmess, limit: kmess}, function(err, result) {
						Promise.all(result.map(function(obj){
							obj = obj._doc;
							delete obj._id;
							return obj;
