...
module.exports = function(client, data){
	...
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			client.VuongQuocRed = void 0 === client.VuongQuocRed ? {id:'', red:red, bet:bet, bonus:null, bonusL:0, bonusWin:0, free:0} :client.VuongQuocRed;
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name daily':'xu name daily', function(err, user){
				if (client.VuongQuocRed.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					...
					if (user.daily != '')
						Helpers.pushDailyVIP({ daily: user.daily, reason: "Cộng tiền người chơi " + user.name + " chơi MỸ NHÂN NGƯ", type: true, total: tongCuoc });
					...
					HU.findOne({game:'vuongquocred', type:bet, red:red}, {}, function(err2, dataHu){
						...
						if (bonusScreen >= 5) {
							// Bonus 5
							client.VuongQuocRed.bonusL = 5;
							gameBonus(client, bet);
						}else if (bonusScreen == 4){
							// Bonus 4
							client.VuongQuocRed.bonusL = 4;
							gameBonus(client, bet);
						}else if (bonusScreen == 3){
							// Bonus 3
							client.VuongQuocRed.bonusL = 3;
							gameBonus(client, bet);
						}
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											red && client.redT.sendInHome({pushnohu:{title:'Mỹ Nhân Ngư', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Mỹ Nhân Ngư', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
							...
							if (red) {
								...
								VuongQuocRed_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({VuongQuocRed:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.VuongQuocRed.free, isFree:isFree, isBonus:client.VuongQuocRed.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:user.red-tongCuoc}});
								...
								VuongQuocRed_xu.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({VuongQuocRed:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.VuongQuocRed.free, isFree:isFree, isBonus:!!client.VuongQuocRed.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong, phien:small.id}, user:{xu:user.xu-tongCuoc}});
									client.VuongQuocRed.id = small._id.toString();
								});
							}
