...
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			//loadCached(bet);
			client.Zeus = void 0 === client.Zeus ? {id:'', red:red, bet:bet, bonus:null, bonusX:0, bonusL:0, bonusWin:0, free:0,lastBet:bet} :client.Zeus;
			client.Zeus.red = red;
			client.Zeus.bet = bet;
			if(client.Zeus.free > 0 && bet != client.Zeus.lastBet){
				client.red({Zeus:{notice:'Bạn không nên đổi cược khi đang free spin'}});
			}else {
				client.Zeus.lastBet = bet;
					...
					HU.findOne({game:'Zeus', type:bet, red:red}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											red && client.redT.sendInHome({pushnohu:{title:'Siêu Xe', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Siêu Xe', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
							...
							if (!!bonusX) {
								client.Zeus.bonusX += bonusX;
								client.Zeus.bonusL = 10;
							...
							if (red) {
								...
								client.red({Zeus:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Zeus.free, isFree:isFree, isBonus:client.Zeus.bonusX, isNoHu:nohu, isBigWin:isBigWin}, user:{red:user.red-tongCuoc}});
								...
								client.red({Zeus:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Zeus.free, isFree:isFree, isBonus:!!client.Zeus.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong}, user:{xu:user.xu-tongCuoc}});
