...
var chat = function(client, str){
	if (!!str) {
		UserInfo.findOne({id:client.UID}, 'red lastVip redPlay vip', function(err, user){
			if (!user || user.red < 0) {
				client.red({taixiu:{err:'Tài khoản phải có ít nhất 1.000 XU để chat.!!'}});
                client = null;
				str = null;
			}else{
...
var cuoc = function (client, data) {
    if (!!data && !!data.bet) {
        if (client.redT.TaiXiu_time <= 5 || client.redT.TaiXiu_time > 60) {
            ...
            if (bet < 5000) {
                client.red({ taixiu: { err: 'Số tiền phải lớn hơn 5000.!!' } });
            } else {
                                ...
                                TXCuocOne.findOne({ uid: client.UID, phien: phien, taixiu: taixiu, red: red }, function (isCuocErr, isCuoc) {
                                    if (!!isCuoc) {
                                        ...
                                        if (isCuoc.select !== select) {
											...
											Push.create({
												type:"GameTXBet",
												data:JSON.stringify({select:select,name:user.name,money:bet,date:new Date()})
											});
                                        }
                                    } else {
                                        // cuoc
