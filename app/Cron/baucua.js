				...
				Promise.all(list.map(function(cuoc){
					...
					if (cuoc[1] > 0) {
						if (void 0 !== heSo[1]) {
							bau = (cuoc[1]*heSo[1]);
							TienThang += bau;
							TongThang += truChietKhau(cuoc[1]+bau, 2);
						}else{
							TongThua  += cuoc[1];
						}
					}
					...
					if (cuoc[2] > 0) {
						if (void 0 !== heSo[2]) {
							ga = (cuoc[2]*heSo[2]);
							TienThang += ga;
							TongThang += truChietKhau(cuoc[2]+ga, 2);
						}else{
							TongThua  += cuoc[2];
						}
					}
					...
					if (cuoc[3] > 0) {
						if (void 0 !== heSo[3]) {
							ca = (cuoc[3]*heSo[3]);
							TienThang += ca;
							TongThang += truChietKhau(cuoc[3]+ca, 2);
						}else{
							TongThua  += cuoc[3];
						}
					}
					...
					if (cuoc[4] > 0) {
						if (void 0 !== heSo[4]) {
							cua = (cuoc[4]*heSo[4]);
							TienThang += cua;
							TongThang += truChietKhau(cuoc[4]+cua, 2);
						}else{
							TongThua  += cuoc[4];
						}
					}
					...
					if (cuoc[5] > 0) {
						if (void 0 !== heSo[5]) {
							tom = (cuoc[5]*heSo[5]);
							TienThang += tom;
							TongThang += truChietKhau(cuoc[5]+tom, 2);
						}else{
							TongThua  += cuoc[5];
						}
					}
	...
	gameLoop = setInterval(function(){
		...
		if (io.BauCua_time <= 60) {
			if (io.BauCua_time < 0) {
				...
				fs.readFile(path.dirname(path.dirname(__dirname)) + '/data/baucua.json', 'utf8', (errjs, bcjs) => {
					try {
						...
						fs.writeFile(path.dirname(path.dirname(__dirname)) + '/data/baucua.json', JSON.stringify(bcjs), function(err){});
				...
				fs.readFile(path.dirname(path.dirname(__dirname)) + '/config/baucua.json', 'utf8', (errcf, bccf) => {
					try {
						...
						if (bccf.bot) {
							...
							UserInfo.find({type:true}, 'id name', function(err, blist){
								if (blist.length) {
									...
									.then(result => {
										let maxBot = (result.length*7/100)>>0;
