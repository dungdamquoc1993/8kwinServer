	...
	HU.findOne({game:'arb', type:bet, red:true}, 'name bet min toX balans x', function(err, dataHu){
		...
		.then(result => {
				...
				if (line_win.type != null) {
					if(line_win.win == 4) {
						...
						if (heso == 100) {
							...
							//io.sendInHome({pushnohu:{title:'Thiên Hà', name:user.name, bet:okHu}});
							HU.updateOne({game:'arb', type:bet, red:true}, {$set:{name:'', bet:quyMin}}).exec();
							AngryBirds_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':bet, 'time':new Date()});
			...
			if (!nohu && bet_win >= bet*10) {
				//io.sendInHome({news:{t:{game:'Thiên Hà', users:user.name, bet:bet_win, status:2}}});
				AngryBirds_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':bet, 'time':new Date()});
			}
