	...
	HU.findOne({game:'bigbabol', type:bet, red:true}, 'name bet min toX balans x', function(err, dataHu){
		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if (line_win.type != null) {
					if(line_win.win == 5) {
						...
						BigBabol_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':bet, 'kq':result.length, 'line':result.length, 'time':new Date()});
						nohu = true;
			...
			if (!nohu && bet_win >= cuoc*2.24) {
				...
				BigBabol_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':result.length, 'time':new Date()});
			}
