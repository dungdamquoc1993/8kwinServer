			...
			result.forEach(function(line_win) {
				if (line_win.win == 6) {
					if (line_win.type === 5) {
						...
						Candy_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				//io.sendInHome({news:{t:{game:'Sex & Zen', users:user.name, bet:bet_win, status:2}}});
