			...
			result.forEach(function(line_win) {
				if(line_win.win == 5) {
					if (line_win.type === 5) {
						...
						//io.sendInHome({pushnohu:{title:'Tam Hùng', name:user.name, bet:okHu}});
						TamHung_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
						nohu = true;
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				//io.sendInHome({news:{t:{game:'Tam Hùng', users:user.name, bet:bet_win, status:2}}});
				TamHung_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
			}
