			...
			result.forEach(function(line_win) {
				if(line_win.win == 5) {
					if (line_win.type === 5) {
						...
						//io.sendInHome({pushnohu:{title:'Mỹ Nhân Ngư', name:user.name, bet:okHu}});

						VuongQuocRed_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				//io.sendInHome({news:{t:{game:'Mỹ Nhân Ngư', users:user.name, bet:bet_win, status:2}}});
				VuongQuocRed_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
			}
