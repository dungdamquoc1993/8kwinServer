	...
	HU.findOne({game:'Zeus', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						//io.sendInHome({pushnohu:{title:'Zeus', name:user.name, bet:okHu}});
						Zeus_red.create({'name':user.name, 'type':bet, 'win':okHu, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
						nohu = true;
			...
			if (!nohu && bet_win >= tongCuoc*2.24) {
				...
				Zeus_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
			}
